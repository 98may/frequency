    ...
    var body: some View {
         VStack {
            HStack {
                Button(action: {
                    self.onDissmis()
                    }) { Text("dismiss")}.padding(20)
                Spacer()
            }

            VStack(alignment: .leading, spacing: 20) {
                Text("habit name").font(.system(size: 28))
                TextField("input your new habit name", text: self.$newItemTitle) {
                ...
                Text("color").font(.system(size: 28))
                HStack(spacing: 4) {
            ...
            VStack {
                ...
                Button(action: {
                    ...
                    Text("add").frame(minWidth: 0, maxWidth: .infinity)
                }
                    .accentColor(.white)
                    .padding()
